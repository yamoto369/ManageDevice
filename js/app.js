...
async function checkNotifications() {
    try {
        ...
        if (result.success) {
            ...
            if (countSpan) {
                countSpan.textContent = `${count} yêu cầu mới`;
            }
            
            if (list && result.data && result.data.length > 0) {
                list.innerHTML = result.data.slice(0, 5).map(req => `
                    <div class="flex gap-3 p-4 hover:bg-gray-50 dark:hover:bg-[#23303e] cursor-pointer border-b border-gray-100 dark:border-gray-700">
                        ...
                        <div class="flex-1">
                            ...
                            <p class="text-xs text-gray-500 mt-1">${timeAgo(req.created_at)}</p>
                        </div>
                    </div>
                `).join('');
            } else if (list) {
