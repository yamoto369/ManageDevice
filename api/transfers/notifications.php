 ...
 * Returns transfer requests that require current user's action
 * - Only requests where current user is the recipient (not initiator)
 * - For transfer type: to_user_id = currentUserId
 * - For borrow_request type: current_holder_id = currentUserId
 */
...
try {
    ...
    // Get only requests that require current user's action
    // Excludes requests initiated by the current user
    $stmt = $db->prepare("
        SELECT tr.*, 
        ...
        WHERE tr.status = 'pending'
          AND tr.from_user_id != :current_user
          AND (
              (tr.type = 'transfer' AND tr.to_user_id = :current_user_transfer)
              OR
              (tr.type = 'borrow_request' AND d.current_holder_id = :current_user_borrow)
          )
        ORDER BY tr.created_at DESC
    ");
    $stmt->execute([
        ':current_user' => $currentUserId,
        ':current_user_transfer' => $currentUserId,
        ':current_user_borrow' => $currentUserId
    ]);
